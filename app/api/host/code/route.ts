...
export async function POST(request: Request) {
  ...
  const booking = await createBooking({
    ...
    channelFeeAmount: Math.max(0, Number(data.channelFeeAmount) || 0),
    touristTaxAmount: Math.max(0, Number(data.touristTaxAmount) || 0),
    ...
    cleaningType: data.cleaningType === "self" ? "self" : "agency",
