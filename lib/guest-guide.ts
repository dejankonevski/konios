...
export type GuestGuide = {
  ...
  wifiName: string; wifiPassword: string; hostName: string; hostPhone: string; parkingSpace: string; parking: string;
...
export const defaultGuestGuide: GuestGuide = {
  ...
  wifiName: "", wifiPassword: "", hostName: "Dejan", hostPhone: "", parkingSpace: "32", parking: "Use the outdoor parking space marked 32.",
...
export async function getGuestGuide() {
  ...
  if (stored.apartmentDirections === undefined) {
    ...
    for (const key of ["address","mapsUrl","floor","apartmentNumber","directions","parkingSpace","buildingCode","buildingEntryInstructions","apartmentDirections","lockboxCode","lockboxInstructions","parking"] as const) {
