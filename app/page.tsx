...
export default async function Home(){
  ...
  return <main className="guest-manual" id="top">
    <header className="manual-header"><a className="brand" href="#top"><span className="brand-mark">K</span><span>{guide.propertyName}</span></a><nav aria-label="Guide sections"><a href="#arrival">Arrival</a><a href="#essentials">Essentials</a>{apartmentInstructions.length>0?<a href="#apartment">Apartment</a>:null}<a href="#explore">Explore</a><a href="#checkout">Checkout</a></nav><span className="manual-stay">{dateLabel(booking.checkIn)} - {dateLabel(booking.checkOut)}</span></header>
    ...
    <section className="manual-section" id="arrival"><div className="manual-heading"><p className="eyebrow">Start here</p><h2>Arrival, step by step.</h2><p>Keep this page open as you approach the building.</p></div><div className="arrival-steps">
      ...
      <article><span>02</span><div><small>Enter the building</small><h3>Building code</h3><div className="access-number">{available(guide.buildingCode)}</div>{guide.buildingCode?<CopyButton value={guide.buildingCode}/>:null}<p className="instruction-lines">{available(guide.buildingEntryInstructions)}</p></div></article>
      ...
      <article><span>04</span><div><small>Find your door</small><h3>{guide.floor?`Floor ${guide.floor}`:"Apartment location"}{guide.apartmentNumber?` · ${guide.apartmentNumber}`:""}</h3><p>{available(guide.apartmentDirections)}</p></div></article>
